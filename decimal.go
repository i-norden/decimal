...
var _pis = [...]Decimal{
	NewFromFloat(7.85398125648498535156E-1),                             // 0x3fe921fb40000000, Pi/4 split into three parts
	NewFromFloat(3.77489470793079817668E-8),                             // 0x3e64442d00000000,
	NewFromFloat(2.69515142907905952645E-15),                            // 0x3ce8469898cc5170,
	NewFromFloat(1.273239544735162542821171882678754627704620361328125), // 4/pi
}
...
func (d Decimal) satan() Decimal {
	...
	return pi.Div(New(4, 0)).Add((d.Sub(New(1, 0)).Div(d.Add(New(1, 0)))).xatan()).Add(NewFromFloat(0.5).Mul(Morebits))
}
...
func (d Decimal) Sin() Decimal {
	...
	j := d.Mul(_pis[3]).IntPart() // integer part of x/(Pi/4), as integer for tests on the phase angle
	...
	z := d.Sub(y.Mul(_pis[0])).Sub(y.Mul(_pis[1])).Sub(y.Mul(_pis[2])) // Extended precision modular arithmetic
	zz := z.Mul(z)

	if j == 1 || j == 2 {
		w := zz.Mul(zz).Mul(_cos[0].Mul(zz).Add(_cos[1]).Mul(zz).Add(_cos[2]).Mul(zz).Add(_cos[3]).Mul(zz).Add(_cos[4]).Mul(zz).Add(_cos[5]))
		y = New(1, 0).Sub(NewFromFloat(0.5).Mul(zz)).Add(w)
	} else {
...
func Ldexp(frac Decimal, exp int) Decimal {
	...
	return frac.Mul(New(2, 0).Pow(NewFromFloat(float64(exp))))
}
